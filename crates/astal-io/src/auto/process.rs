...
impl Process {
    ...
    pub fn new(cmd: &[&str]) -> Result<Process, glib::Error> {
        ...
        unsafe {
            ...
            let ret = ffi::astal_io_process_new(cmd.to_glib_none().0, cmd_length1, &mut error);
    ...
    pub fn subprocessv(cmd: &[&str]) -> Result<Process, glib::Error> {
        ...
        unsafe {
            let mut error = std::ptr::null_mut();
            let ret = ffi::astal_io_process_subprocessv(cmd.to_glib_none().0, cmd_length1, &mut error);
            if error.is_null() { Ok(from_glib_full(ret)) } else { Err(from_glib_full(error)) }
        }
    }
...
pub trait ProcessExt: IsA<Process> + 'static {
    ...
    fn connect_stdout<F: Fn(&Self, &str) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, c"stdout".as_ptr() as *const _,
    ...
    fn connect_stderr<F: Fn(&Self, &str) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, c"stderr".as_ptr() as *const _,
    ...
    fn connect_exit<F: Fn(&Self, i32, bool) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            let f: Box_<F> = Box_::new(f);
            connect_raw(self.as_ptr() as *mut _, c"exit".as_ptr() as *const _,
