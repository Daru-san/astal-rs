...
pub trait ApplicationExt: IsA<Application> + 'static {
    ...
    fn request(&self, request: &str, conn: &impl IsA<gio::SocketConnection>) -> Result<(), glib::Error> {
        unsafe {
            let mut error = std::ptr::null_mut();
            let _ = ffi::astal_io_application_request(self.as_ref().to_glib_none().0, request.to_glib_none().0, conn.as_ref().to_glib_none().0, &mut error);
            if error.is_null() { Ok(()) } else { Err(from_glib_full(error)) }
        }
    }
    ...
    fn instance_name(&self) -> glib::GString {
        unsafe {
    ...
    fn connect_instance_name_notify<F: Fn(&Self) + 'static>(&self, f: F) -> SignalHandlerId {
        ...
        unsafe {
            ...
            connect_raw(self.as_ptr() as *mut _, c"notify::instance-name".as_ptr() as *const _,
